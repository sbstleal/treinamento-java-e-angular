...
public class OperacaoContaService extends GenericCrudService<OperacaoConta, Long, OperacaoContaRepository>{
	...
	public List<OperacaoContaDTO> obterOperacoesData(String agencia, String numeroConta, String dataInicio, String dataFim) {
		ContaBancaria conta = contaBancariaService.consultarConta(agencia, numeroConta);
		DateTimeFormatter parser = DateTimeFormatter.ofPattern("dd-MM-yyyy");
		LocalDateTime dataInicial = LocalDate.parse(dataInicio, parser).atStartOfDay();
		LocalDateTime dataFinal = LocalDate.parse(dataFim, parser).atTime(23, 59, 59);
		
		List<OperacaoConta> operacoes = operacaoContaRepository.findByContaAndDataHoraBetween(conta, dataInicial, dataFinal);
		if(operacoes == null || operacoes.isEmpty()) {
			throw new AplicacaoException(ExceptionValidacoes.ALERTA_NENHUM_REGISTRO_ENCONTRADO);
		}
		List<OperacaoContaDTO> listarResultado = new ArrayList<>();
